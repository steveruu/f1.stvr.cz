...
const JOLPICA_BASE_URL = 'https://api.jolpi.ca/f1';
  ...
  useEffect(() => {
    const fetchSchedule = async () => {
      try {
        ...
        const response = await fetch(`${JOLPICA_BASE_URL}/${CURRENT_YEAR}.json`);
  ...
  useEffect(() => {
    const fetchDriverStandings = async () => {
      try {
        ...
        const response = await fetch(`${JOLPICA_BASE_URL}/${CURRENT_YEAR}/driverStandings.json`);
        ...
        if (!data || !data.MRData || !data.MRData.StandingsTable || !data.MRData.StandingsTable.StandingsLists || 
  ...
  useEffect(() => {
    const fetchConstructorStandings = async () => {
      try {
        ...
        const response = await fetch(`${JOLPICA_BASE_URL}/${CURRENT_YEAR}/constructorStandings.json`);
